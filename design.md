...
- **Per entity**:
  - `entity.type == "character"` — find the owning player (linear scan over `game.players` matching `p.character == entity`). Move the player into a runtime permission group `hmfea-petrified-player` that disables **every** `defines.input_action` and re-enables only a tiny allow-list (open / close GUI for map access, chat, blueprint library, kills / train / logistic / technology GUIs, edit permission group). The character is **not** killed — it stays in the world, frozen, perfectly visible to other players. Storage flag `storage.petrificus.players[player_index] = true` so `Petrificus.on_player_joined_game` re-adds the player to the group on rejoin.
...
- **Why permission group instead of god controller**: the requirement is *"radar only, nothing more"*. The god / spectator controller gives free-flight everywhere — the opposite of the intent. The permission-group lockdown keeps the player's character pinned in place; the only actions that succeed are the ones in the allow-list, and those are all read-only (open the map, browse blueprints, chat).
